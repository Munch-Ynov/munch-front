<template>
  <div class="m-8 flex flex-row p-5 justify-center items-start">
    <form @submit="submit" class="card flex flex-col gap-8">
      ...
      <div>
        ...
        <div class="flex flex-col gap-4">
          ...
          <div class="flex flex-col gap-2">
            ...
            />
            <p v-if="passwordConfirmation" class="form-error">
      ...
      <RouterLink
        to="/connexion"
        class="text-sm font-semibold text-center cursor-pointer"
...
const validationSchema = toFormValidator(
  z
    .object({
      firstName: z
        .string({ required_error: "Vous devez renseigner ce chmap" })
        .min(2, "Trop court"),
      lastName: z
        .string({ required_error: "Vous devez renseigner ce chmap" })
        .min(2, "Trop court"),
      ...
      password: z
        ...
        .min(5, "Le mdp doit faire au moins 5 caractères"),
...
const submit = handleSubmit(async (formValue: UserForm) => {
  ...
  const formValueWithoutPasswordConfirmation = {
  ...
  try {
    await createUser(formValueWithoutPasswordConfirmation);
...
const { value: passwordValue, errorMessage: passwordError } =
  useField("password");
const { value: passwordConfirm, errorMessage: passwordConfirmation } = useField(
  "passwordConfirmation"
);
</script>
