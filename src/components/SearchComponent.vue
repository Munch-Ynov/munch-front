  ...
  <div>
    <form @submit="submit">
      <div class="flex flex-col gap-1">
        <div
          id="searchBarWrapper"
          class="px-7 flex items-center relative h-10 gap-4"
        >
          <button class="rounded-lg absolute cursor-pointer mx-2 text-xl">
          ...
          <input
            ...
            id="search"
            type="text"
            class="bg-[#F4F4F4] pl-8 border-none w-full"
          />
          <!-- button on click router.push("/search") -->
          <button
            class="rounded-lg cursor-pointer text-xl bg-[#F4F4F4] relative flex items-center justify-center h-full px-4"
            @click="router.push('/search')"
          >
            <Icon class="absolute" icon="mdi:filter-outline" />
          </button>
        </div>
    ...
    <div v-if="vignettesTable.length" class="flex gap-4 pb-6 px-7">
      <div
        v-for="vignette in vignettesTable"
        :key="vignette.value"
        class="flex items-center gap-2 bg-[#81C1A6] rounded-2xl px-4 py-1 font-bold text-white"
      >
        <span>{{ vignette.value }}</span>
        <button @click="removeVignette(vignette)" class="cursor-pointer">
          <Icon icon="mdi:close" />
        </button>
      </div>
    </div>
...
const validationSchema = toFormValidator(
  z.object({
    search: z
      .string()
      .min(3, "La recherche doit contenir au moins 3 caractères"),
  })
);
...
const submit = handleSubmit((value) => {
  ...
  restaurantStore.filterSearch(value.search).then(() => {
    if (vignettesTable.length === 0) {
      ...
      const isSearchFilterPresent = vignettesTable.find(
        (vignette) => vignette.type === "search"
      );
